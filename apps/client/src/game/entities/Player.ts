...
export class Player {
  ...
  update(deltaMs: number): void {
    ...
    const next = step(this.state, input, deltaMs, this.bounds);
    if (next !== this.state) {
      this.state = next;
      this.draw();
    }
  ...
  /**
   * Plays a tool swing once, in the current facing, across every layer.
   *
   ...
   * (character frozen holding a raised hoe).
   *
   * Cosmetic, like everything else here. It gates nothing and tells the server
   * nothing; T-9.06 wires it to the real intents, which the server validates on
   * their own terms (§4.1).
   */
  playToolAnimation(kind: 'hoe' | 'water'): void {
    ...
    const anim: ToolAnim = kind === 'hoe' ? 'hoe' : 'watering';
  ...
  private readInput(): MoveInput {
    ...
    if (isTypingInDom()) return NO_INPUT;
  ...
  private draw(): void {
    ...
    this.container.setDepth(DEPTH.world + y);
